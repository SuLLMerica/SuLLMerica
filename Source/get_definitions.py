            ...
            if "Terms and definitions" in text:
                ...
                if processing_terms and ":" in text:
                    ...
                    if len(abbreviation) > 1:
                        abbreviations_definitions[abbreviation.strip()] = (
                            definition.strip()
                        )
...
def find_and_filter_terms(terms_dict, sentence):
    ...
    matched_terms = {
        ...
        if preprocess(term) in lowercase_sentence
    }
...
def find_and_filter_abbreviations(abbreviations_dict, sentence):
    ...
    matched_abbreviations = {
        word: abbreviations_dict[word] for word in words if word in abbreviations_dict
    }
...
def find_terms_and_abbreviations_in_sentence(terms_dict, abbreviations_dict, sentence):
    ...
    formatted_terms = [
        f"{term}: {definition}" for term, definition in matched_terms.items()
    ]
    formatted_abbreviations = [
        f"{abbr}: {definition}" for abbr, definition in matched_abbreviations.items()
    ]

    return formatted_terms, formatted_abbreviations


def find_terms_and_abbreviations_and_filter(terms_dict, abbreviations_dict, sentence):
    ...
    """
    processed_sentence = preprocess(
        sentence, lowercase=False
    )  # Preserve case for abbreviations
    matched_abbreviations = {
        abbr: abbreviations_dict[abbr]
        for abbr in abbreviations_dict
        if abbr in processed_sentence
    }
